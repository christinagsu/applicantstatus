...
<cfif isDefined("URL.schol_id")>
    ...
    <cfloop query="out_result_set_scholtype">
        <cfif (scholarship_id eq URL.schol_id and (scholarship_code eq "" or scholarship_code eq URL.schol_code)) or scholarship_code eq URL.schol_code><cfset scholtype=scholarship_type><cfset scholcode=scholarship_code><cfset scholamount=scholarship_amount><cfset scholprojid=project_id><cfset scholdesc=scholarship_description><cfset gpatext=scholarship_gpa_req></cfif>
    </cfloop>
    <cfif scholtype eq "">
    ...
    <cfelse>
        ...
        select * from applications, students where students.student_id=applications.student_id and gsu_student_id=#studid# <cfif isDefined("schol_id") and schol_id neq "">and scholarship_id=#schol_id#</cfif> and awarded='y' order by application_start_date desc
        </cfquery>
        <cfif isDefined("schol_id") and schol_id neq "">
...
<cfelse>
    ...
    <cfif isDefined("scholtype") and scholtype eq "U" and schol_exception eq false>
    ...
    <cfif isDefined("scholtype") and scholtype eq "U"> 
	 ...
	 <cfif scholcode eq "HSR" or schol_code eq "HSR2">
	      <cfset form_text=#Replace(form_text, "[ITEM 4 GPA]", 3.3, "all")#>
         <cfelseif scholcode eq "EXA">
	      <cfset form_text=#Replace(form_text, "[ITEM 4 GPA]", 3.0, "all")#>
       <cfelse>
	      <cfset form_text=#Replace(form_text, "[ITEM 4 GPA]", "", "all")#>
	 </cfif>
	 <cfset form_text=#Replace(form_text, "[]")
	 ...
	 <cfif isDefined("URL.schol_code")>
	      <cfquery name="getContactScholInfo" datasource="scholarships">
	      ...
	      <cfquery name="getContactInfo" datasource="scholarships">
		     select * from contact_info where contact_unit='#trim(getContactScholInfo.awarding_unit)#'
	      </cfquery>
     ...
     <cfif isDefined("URL.schol_code")>
       <cfquery name="getDeadline" datasource="eAcceptance">
	      select * from custom_paragraphs where paragraph_type='scholarshipDeadline#trim(getContactScholInfo.awarding_unit)#'
       </cfquery>
       <cfset form_text=#Replace(form_text, "[DEADLINE DATE]", #getDeadline.paragraph_text#, "all")#>
    </cfif>
     <cfif scholtype eq "S" or schol_exception eq true><cfset itemtype="SR"><cfelseif scholtype eq "U"><cfset itemtype="UW"></cfif>
     <cfif scholtype eq "U" and schol_exception eq false>
