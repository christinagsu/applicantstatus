<CFFUNCTION NAME="showLoginPage">
	...
	<cfoutput>
	<H2>Login</H2>
	<p>Thank you for visiting the Admissions Status Check. Here you will find out if any additional information or documentation is necessary for your application to be considered by the Office of Undergraduate Admissions for both our Perimeter College and Atlanta Campus. If you are a student who has previously applied for both an associate's and bachelor's degree, please note that the ID you enter will only lead you to information pertaining to that corresponding degree track. </p>

	<p>Students who enter log-in information and see a resulting message stating that their application cannot be found should contact the Office of Admissions at the appropriate school through either <a href="mailto:gpcrec@gpc.edu">gpcrec@gpc.edu</a> or <a href="mailto:admissions@gsu.edu">admissions@gsu.edu</a>. If you are unable to log-in after providing all of the correct information, please make sure that you have allowed 24- 48 hours for your application to be downloaded into our student information system. </p>

	<p>Panther IDs and GPC User IDs can be found in the first email you received after opening an application with either school or you can search for it below. </p>
...
class="formw">
        <INPUT TYPE="text" NAME="stud_four_digits_ssn" id="stud_four_digits_ssn" SIZE="12" MAXLENGTH="4" value="<cfif isDefined('Form.stud_four_digits_ssn')>#Form.stud_four_digits_ssn#</cfif>">
		 <span class="extrarow">- OR -</span><span class="extrarow"> Complete Panther##: </span><span class="extrarow"><INPUT TYPE="text" NAME="stud_panther_number" id="stud_panther_number" SIZE="9" MAXLENGTH="9" value="<cfif isDefined('Form.stud_panther_number')>#Form.stud_panther_number#</cfif>"></span>
...
class="formw">
		 ...
		 <span class="extrarow"><A HREF="" onclick="return open_gsuidwin()" style="margin-left: 156px;">What is my Panther ##?</a>  </span><span class="extrarow"><a href="" onclick="return open_gpcidwin()" style="margin-left: 105px;">What is My GPC-ID?</a></span>
      </span>
    </div>
    </div>
  <div class="spacer">
...
<CFFUNCTION NAME="tryLogin">
	...
	<cfif ssn neq "????" or Form.stud_gpc_id neq "">
		...
		<cfif (ssn neq "" and ssn neq "????") or (pnum neq "" and pnum neq "?????????")>
		<cfif ssn eq "????"><cfset tempssn=""><cfelse><cfset tempssn=ssn></cfif>
			<cfstoredproc  procedure="wwokbapi.f_get_adm_id" datasource="#Session.odatasource#">
			<cfprocparam cfsqltype="CF_SQL_VARCHAR" dbvarname="in_bmonth" type="in" value="#Form.birth_month#"> 
			<cfprocparam cfsqltype="CF_SQL_VARCHAR" dbvarname="in_bday" type="in" value="#Form.birth_day#">
			<cfprocparam cfsqltype="CF_SQL_VARCHAR" dbvarname="in_byear" type="in" value="#Form.birth_year#"> 
			<cfprocparam cfsqltype="CF_SQL_VARCHAR" dbvarname="in_ssn" type="in" value="#ssn#"> 
			<cfprocparam cfsqltype="CF_SQL_VARCHAR" dbvarname="in_pnum" type="in" value="#pnum#">
			<cfprocparam cfsqltype="CF_SQL_VARCHAR" dbvarname="in_fname" type="in" value="#Form.stud_first_name_three_letters#"> 
			<cfprocparam cfsqltype="CF_SQL_VARCHAR" dbvarname="in_lname" type="in" value="#studlastname#">  
	...
	<cfcatch>
		...
		<h3 style="color:red;">Please try back later. Thank you.</h3>--->
		  <cfinvoke method="showLoginPage" /><cfexit>
	</cfcatch>
	...
	<cfif (not isDefined("getResult") or not isDefined("getResult.Authentication") or #getResult.Authentication# neq "SUCCESSFUL") and (not isDefined("out_result_set.panther_no") or out_result_set.panther_no eq "")>
		...
		<cfelse>
			<p class="errortext" style="color:red;"><i>Your login information did not match correctly.</i></p>
...
and   spriden_pidm =
  ...
  <cfhttpparam name="Content-Type" value="application/x-www-form-urlencoded" type="header">
    ...
    <cfhttpparam type="body" value='{"authentication": {"api_key": "GSU783354110","api_pass": "A|7E>arpK.L8:qkAX$ab"},"applicantStatus": {"studentId": "#gpcid#"}}'>
	 ...
	 <cfif isDefined("getResult.applicationStatus") and not (getMetadata(getResult.applicationStatus).getName() eq "coldfusion.runtime.Struct" and getResult.applicationDate eq "null") and not (isDefined("getResult.applicationDate") and getResult.applicationDate eq "null")><cfset gpc_record=true></cfif>
	...
	<cfif pid eq "">
		<cfquery name="getGSUID" datasource="hsguidanceoracle">
		select spriden_id
			from spriden
			where spriden_pidm =
					 (select spriden_pidm
						from spriden
						where spriden_id = '#gpcid#'
						and   spriden_id like '9%'
						and   spriden_ntyp_code = 'LGCY'
						and   spriden_change_ind = 'I')
			and   spriden_change_ind is null
			union
	   ...
	   <cfset scholid="">
		...
		<cfset gpatext=scholarship_gpa_req>
            ...
            <cfif nonelisted eq true>
                <cfset final_text=Replace('#scholText#', '[AWARDED SCHOLARSHIPS]', '<i>None are available to be accepted at this time.</i>', 'all')>
            <cfelse>
